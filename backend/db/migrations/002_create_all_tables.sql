...
ALTER TABLE users ADD CONSTRAINT check_cedula_length CHECK (length(cedula) <= 20);
ALTER TABLE users ADD CONSTRAINT check_email_length CHECK (length(email) <= 100);
ALTER TABLE users ADD CONSTRAINT check_password_positive CHECK (length(password_hash) > 0);

-- Courses table constraints
ALTER TABLE courses ADD CONSTRAINT check_title_length CHECK (length(title) <= 100);
ALTER TABLE courses ADD CONSTRAINT check_description_length CHECK (length(description) <= 200);
ALTER TABLE courses ADD CONSTRAINT check_long_description_length CHECK (length(long_description) <= 1000);
ALTER TABLE courses ADD CONSTRAINT check_hours_range CHECK (hours >= 0 AND hours <= 99999);

-- Profiles table constraints
ALTER TABLE profiles ADD CONSTRAINT check_profiles_name_length CHECK (length(name) <= 100);
ALTER TABLE profiles ADD CONSTRAINT check_username_length CHECK (length(username) <= 50);
ALTER TABLE profiles ADD CONSTRAINT check_profiles_email_length CHECK (length(email) <= 100);
ALTER TABLE profiles ADD CONSTRAINT check_bio_length CHECK (length(bio) <= 250);
